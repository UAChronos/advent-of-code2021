...
public class Main
{
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        for(int i = 0; i < drawOrder.length && counter < boards.size(); i++)
        {
            //For every set of boards until last
            for (int j = 0; j < boards.size(); j++)
            {
                ...
                {
                    if(!boards.get(j).getWinState())
                    {
                        counter++;
                        points = boards.get(j).countPoints(drawOrder[i]);
                    }
                }
            }
        }
